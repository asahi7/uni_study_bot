/*
 * Never pass your credentials with this class.
*/
public class Consts
{
    public static final String TOKEN = "<your-token>";
    public static final String USERNAME = "uni_study_bot";
    public static final String LINK_DB = "jdbc:mysql://localhost:3306/uni_study_bot?useUnicode=true&characterEncoding=UTF-8&autoReconnect=true";
    public static final String USER_DB = "<your-db-user>";
    public static final String PASSWORD_DB = "<your-db-password>";
}